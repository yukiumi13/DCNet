...
def train(data):
    ...
    min_dice = 0.8
    min_dice_val = 0
    writer = tb.SummaryWriter('./events_ssm_c')
    train_idx = 0 
    for epoch in range(300):
        ...
        if  dice_val > min_dice_val : 
            ...
            print('保存参数 ')
            torch.save(ssm.state_dict(), "./parameters/" + "best" + '_c_' + str(min_dice_val) + ".pth")
...
def test(data):
    ...
    for i, (imagedata, labeldata) in enumerate(data):
        ...
        gt_img = gt_img[0,:,:]
        gt_img_1 = torch.squeeze(gt_img)
        dice = dice_unweighted(yp_img_1,gt_img_1)
