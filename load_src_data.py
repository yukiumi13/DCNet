...
def load_labeled_data_one(path, images_path, masks_path):
    ...
    for file in files_mask: 
        ...
        mask = cv2.imread(file_path_mask) #加载对应文件至image mask变量
        mask = cv2.cvtColor(mask, code=cv2.COLOR_BGR2GRAY) # 将图片转换到另一彩色空间
        ...
        th, mask_r = cv2.threshold(mask_r, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU) #阈值处理
        mask_r = np.reshape(mask_r,newshape=[cg.fix_size,cg.fix_size,1]) # reshape为三维张量

        shapes.append( [resized_shape,src_shape]) 
        images.append(image_r)
        masks.append(mask_r) # 分别将结果存储在shapes，iamges，masks
    ...
    shapes = np.array(shapes,dtype=np.int32) # 标准化为uint8格式


    print("images shape=", images.shape, "masks shape=", masks.shape,'shapes shape=',shapes.shape)
    print("images dtype=", images.dtype, "masks dtype=", masks.dtype,'shapes dtype=',shapes.dtype）# datatype
...
if __name__ == '__main__': # 防止模块读取时调用
